...
import pandas as pd
def predict_fetal_health(data):
  """
  This function takes a list of features as input and predicts the fetal health using the trained Gradient Boosting Classifier model.

  Args:
    data: A list containing the following features:
      - baseline value
      - accelerations
      - fetal_movement
  ...
  Returns:
    The predicted fetal health (1 or 2).
  """
  ...
  data_scaled = s_scaler.transform(data_df)

  # Predict the fetal health using the trained model
  prediction = gbcl_mod.predict(data_scaled)[0]

  return prediction

# prompt: call the above function and give data?

features = [0.539358, 0.0, 0.276073, 0.368627, 0.0, 0.0, 0.0, 0.326488, 0.359754, 0.0, 1.304348]
predicted_health = predict_fetal_health(features)
